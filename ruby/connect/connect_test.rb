...
class ConnectTest < Minitest::Test
  ...
  def test_x_can_win_on_a_2x2_board
    board = [
      '. X',
      ' X .'
    ].map { |row| row.gsub(/^ */, '') }
    game = Board.new(board)
    assert_equal 'X', game.winner, 'X can win on a 2x2 board'
  end
  ...
  def test_o_can_win_on_a_2x2_board
    board = [
      '. O',
      ' . O'
    ].map { |row| row.gsub(/^ */, '') }
    game = Board.new(board)
    assert_equal 'O', game.winner, 'O can win on a 2x2 board'
  end
  ...
  def test_o_wins_using_convoluted_path
    board = [
      '. O . .',
      ' O X O O',
      '  O O X O',
    ...
    game = Board.new(board)
    assert_equal 'O', game.winner, 'O wins using convoluted path'
  end
